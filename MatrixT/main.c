...
int main(void)
{
   ...
   for(int i = 0; i < MATRIX_SIZE; i++){
      ...
      pthread_t *threads;
      thread_args *args;
      if ((threads = malloc(sizeof(pthread_t) * (unsigned long) MATRIX_SIZE)) == NULL ||
        (args = malloc(sizeof(thread_args) * (unsigned long) MATRIX_SIZE)) == NULL)
      {
        exit(EXIT_FAILURE);
      }
      ...
      for (int i = 0; i < MATRIX_SIZE; i++)
      {
        ...
        pthread_create(&threads[i], NULL, (void *) &row_multiply, (void *) &args[i]);
      }
